...
const Heading: React.FC<HeadingInterface> = ({ type, children }) => {
  function getHeaderComponent() {
    switch (type) {
      ...
      case HeaderType.h4:
        return <h4 className={classnames(styles.title, styles.h4)}>{children}</h4>;
      case HeaderType.h5:
        return <h5 className={classnames(styles.title, styles.h5)}>{children}</h5>;
      case HeaderType.h6:
        return <h6 className={classnames(styles.title, styles.h6)}>{children}</h6>;
      default:
        return <h1 className={classnames(styles.title, styles.h1)}>{children}</h1>;
    }
  }
